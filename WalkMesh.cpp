...
void WalkMesh::walk_in_triangle(WalkPoint const &start, glm::vec3 const &step, WalkPoint *end_, float *time_) const {
    ...
    { //project 'step' into a barycentric-coordinates direction:
		...
		//step_coords = barycentric_weights(a, b, c, end_pos);
        end.weights = barycentric_weights(a, b, c, end_pos);
        step_coords = end.weights - start.weights;
    ...
    glm::vec3 velocity = end.weights - start.weights;
    glm::vec3 times = start.weights / (- velocity);

    float time_min = time;
    if (times.x > 0.0f && times.x < time_min) {
        time_min = times.x;
    }
    if (times.y > 0.0f && times.y < time_min) {
        time_min = times.y;
    }
    if (times.z > 0.0f && times.z < time_min) {
        time_min = times.z;
    }

    end.weights = start.weights + step_coords * time_min;
	time = time_min;

    if (times.x != 1.0f && times.x == time_min) {
        end.weights = glm::vec3(end.weights.y / (end.weights.y + end.weights.z),
								end.weights.z / (end.weights.y + end.weights.z),
								0.0f
		);
		end.indices = glm::uvec3(start.indices.y, start.indices.z, start.indices.x);
    } 
	else if (times.y != 1.0f && times.y == time_min) {
        end.weights = glm::vec3(end.weights.z / (end.weights.x + end.weights.z),
								end.weights.x / (end.weights.x + end.weights.z),
								0.0f
		);
		end.indices = glm::uvec3(start.indices.z, start.indices.x, start.indices.y);
    } 
	else if (times.z != 1.0f && times.z == time_min) {
        end.weights = glm::vec3(end.weights.x / (end.weights.x + end.weights.y),
								end.weights.y / (end.weights.x + end.weights.y),
								0.0f
		);
...
bool WalkMesh::cross_edge(WalkPoint const &start, WalkPoint *end_, glm::quat *rotation_) const {
    ...
    auto tmp_next_vertex = next_vertex.find(edge);
    if (tmp_next_vertex != next_vertex.end()) {
        //it is!
        ...
        end.indices.z = tmp_next_vertex->second;

        //make 'rotation' the rotation that takes (start.indices)'s normal to (end.indices)'s normal:
        //TODO
        glm::vec3 to_normal = glm::normalize(glm::cross(vertices[start.indices.y] - vertices[start.indices.x], 
                                                        vertices[start.indices.z] - vertices[start.indices.x]));
        glm::vec3 end_normal = glm::normalize(glm::cross(vertices[end.indices.y] - vertices[end.indices.x], 
                                                         vertices[end.indices.z] - vertices[end.indices.x]));
        rotation = glm::rotation(to_normal, end_normal);
